...
@Explain(name = "客户优惠券管理", notes = "客户优惠券管理")
public class CustomCouponService {

    @Autowired
    private ICustomCouponDao customCouponDao;

    @AdminAuth(name = "客户优惠券列表", orderNum = 1)
    @ExplainOperation(name = "客户优惠券列表", notes = "客户优惠券列表", params= {
             @ExplainParam(value = "page", name = "页码，从0开始，默认0", require = false, type = "int", example = "0"),
             @ExplainParam(value = "size", name = "每页条数，默认15答", require = false, type = "int", example = "15"),
             @ExplainParam(value = "sort", name = "排序，id_desc表示根据id降序", require = false, type = "String", example = "id_desc"),
             @ExplainParam(value = "conditions", name = "筛选条件，id_eq:5表示id=5", require = false, type = "String", example = "id_eq:5")
     }, back = {
             @ExplainReturn(field = "size", type = "int", notes = "客户优惠券数量"),
             @ExplainReturn(field = "datas", type = "Object", notes = "客户优惠券数组对象")
     })
    ...
    @AdminAuth(name = "获取客户优惠券", orderNum = 5)
    @ExplainOperation(name = "获取客户优惠券信息", notes = "通过ID获取角色对象", params = {
            @ExplainParam(value = "id", name = "客户优惠券ID", require = true, type = "int", example = "1")
    }, back = {
    ...
    public JsonResult loadOne(String params) {
        try {
            ...
            CustomCoupon obj = customCouponDao.findOne(id);
